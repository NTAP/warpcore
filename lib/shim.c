...
// static SLIST_HEAD(sh, w_sock) sock;  // our open sockets
...
{
    int * s;
    assert(s = calloc(1, sizeof(*s)), "calloc");
    assert(*s = socket(PF_INET, SOCK_DGRAM, p), "socket");
    assert(fcntl(*s, F_SETFL, O_NONBLOCK) != -1, "fcntl");

    // make a sockaddr
    struct sockaddr_in addr = {.sin_family = AF_INET,
                               .sin_port = port,
                               .sin_addr = {htonl(INADDR_ANY)}};
    assert(bind(*s, (struct sockaddr *)&addr, sizeof(addr)) == 0, "bind");
...
w_connect(struct w_sock * const s, const uint32_t ip, const uint16_t port)
{
    ...
    assert(connect(*(int *)(void *)s, (struct sockaddr *)&addr, sizeof(addr)) !=
               -1,
           "socket");
}
...
// w_tx_alloc(struct w_sock * const s __attribute__((unused)),
...
struct w_iov * __attribute__((nonnull))
w_tx_alloc(struct w_sock * const s __attribute__((unused)), const uint32_t len)
{
    assert(len, "len is zero");

    // add enough buffers to the iov so it is > len
    STAILQ_HEAD(ov, w_iov) ov = STAILQ_HEAD_INITIALIZER(ov);
    struct w_iov * v = 0;
    ...
    while (l > 0) {
        ...
        v->len = 1400; // s->w->mtu - sizeof(s->hdr);
        l -= v->len;
        n++;

        // add the iov to the tail of the result
        STAILQ_INSERT_TAIL(&ov, v, next);
    }
...
void __attribute__((nonnull))
w_rx_done(struct w_sock * const s __attribute__((unused)))
{
    warn(warn, "here");
...
{
    STAILQ_HEAD(iv, w_iov) iv = STAILQ_HEAD_INITIALIZER(iv);
    ssize_t n = 0;

    do {
        // grab a spare buffer
        struct w_iov * v = STAILQ_FIRST(&iov);
        assert(v != 0, "out of spare bufs");
        warn(debug, "grabbing spare buf %u for user rx", v->idx);
        n = recv(*(int *)s, v->buf, IOV_BUF_LEN, 0);
        ...
        // add the iov to the tail of the result
        STAILQ_REMOVE_HEAD(&iov, next);
        STAILQ_INSERT_TAIL(&iv, v, next);

        // store the length
        v->len = (uint16_t)n;
        warn(warn, "received %zu byte%c", n, plural(n));
    } while (n);
    return STAILQ_FIRST(&iv);
}
...
void __attribute__((nonnull))
w_tx(struct w_sock * const s __attribute__((unused)))
{
    warn(warn, "here");
}
